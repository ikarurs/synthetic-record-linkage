...
The outcome notebook demonstrates three comparisons: [ranked decision shares with counts and denominators](outputs/fictional_decisions.png), [housing rates on identical town panels](outputs/fictional_outcomes.png), and [the sensitivity of a comparison to pending linkage reviews](outputs/fictional_sensitivity.png). Direct labels, hatching and open/filled markers preserve meaning without relying on colour. Every figure has a scalable SVG export and is regenerated by `outcome_visualise.py` from the fictional input CSVs.

![Housing rates for six invented towns on identical scales. Fictional 04 has a missing Year 2 observation; no value is imputed.](outputs/fictional_outcomes.png)
